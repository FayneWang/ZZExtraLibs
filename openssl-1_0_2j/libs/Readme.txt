...
3、In the ./ms/nt(dll).mak of built,modify the zlib1.lib to zlib.lib for using static library of zlib.and then copy zlib.h and zlib.lib of static library to root directory,
4、Modify /MT(d) of CFLAGS variable to /MD(d) in file of ./ms/nt(dll).mak file,  in root dir of lib for no-share compile.